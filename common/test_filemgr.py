...
def test_split_and_reassemble():
    ...
    try:
        ...
        reassembled_path = fm.reassemble(
            output_path=reassembled_file, input_dir=output_dir
        )
...
def test_compress_and_split():
    """Test the compress_and_split operation."""
    # Create test data
    test_dir = Path(tempfile.mkdtemp(prefix="test_split_"))
...
def test_temp_dir_context_manager():
    ...
    try:
        ...
        assert not temp_dir_path.exists(), (
            "Temp dir should be deleted after context exit"
        )
...
def test_temp_dir_without_context_manager():
